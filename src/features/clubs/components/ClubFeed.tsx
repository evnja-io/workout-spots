...
export function ClubFeed({
  ...
  const { data: posts = [], isPending } = useQuery({
    ...clubFeedQueryOptions(clubId, userId),
    enabled: !locked,
  })

  const createPost = useCreatePost(clubId)
  const toggleLike = useToggleLike(clubId)
  const deletePost = useDeletePost(clubId)
  const addComment = useAddComment(clubId)
  const deleteComment = useDeleteComment(clubId)

  if (locked) {
    return (
      <div className="flex flex-col items-center gap-2 rounded-xl border border-dashed border-border px-6 py-10 text-center">
        <Icon name="users" size={24} />
        <p className="max-w-xs text-[13px] text-text-3">{t('clubs.feedLocked')}</p>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      {canCompose && <Composer pending={createPost.pending} onSubmit={createPost.submit} />}
    ...
    <div className="rounded-[18px] border border-border bg-surface p-3.5">
      ...
      <div className="mt-2 flex items-center justify-between">
        <label className="inline-flex cursor-pointer items-center gap-1.5 text-[13px] text-text-3 hover:text-text">
          ...
          {file ? <span className="max-w-[140px] truncate">{file.name}</span> : t('clubs.addPhoto')}
          <input
            type="file"
            accept="image/*"
            className="hidden"
            ...
            return (
              <div key={c.id} className="flex items-start gap-2">
                ...
                {own && (
                  <button
                    type="button"
                    aria-label={t('clubs.deleteComment')}
          ...
          {canComment && (
            <div className="flex items-center gap-2">
              <input
                ...
                placeholder={t('clubs.commentPlaceholder')}
                maxLength={1000}
